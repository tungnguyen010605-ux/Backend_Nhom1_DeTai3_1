...
def _cleanup_strict(cur: sqlite3.Cursor) -> None:
    ...
    # Remove users with physically unreasonable measurements and their related records.
    cur.execute(
        """
        CREATE TEMP TABLE unreasonable_users AS
        SELECT id
        FROM user_profiles
        WHERE height_cm < 120 OR height_cm > 230
           OR chest_cm < 50 OR chest_cm > 160
           OR waist_cm < 45 OR waist_cm > 150
           OR hip_cm < 60 OR hip_cm > 170
           OR inseam_cm < 45 OR inseam_cm > 120
           OR inseam_cm >= height_cm
           OR chest_cm > height_cm * 0.95
           OR waist_cm > height_cm * 0.9
           OR hip_cm > height_cm * 0.95
        """
    )
    cur.execute(
        """
        INSERT OR IGNORE INTO unreasonable_users (id)
    ...
    cur.execute("DELETE FROM body_measurements WHERE user_id IN (SELECT id FROM unreasonable_users)")
    cur.execute("DELETE FROM clothing_items WHERE user_id IN (SELECT id FROM unreasonable_users)")
    cur.execute("DELETE FROM user_profiles WHERE id IN (SELECT id FROM unreasonable_users)")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Apply stronger cleanup: remove users without measurements, "
            "deduplicate by name, and drop unreasonable body-proportion records."
        ),
    )
