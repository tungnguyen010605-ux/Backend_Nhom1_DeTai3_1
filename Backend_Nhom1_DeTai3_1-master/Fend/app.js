...
const deleteClothingBtn = document.getElementById("delete-clothing");
...
function clearUserMeasurementFields() {
...
async function loadUsers() {
  const prevSelectedId = Number(existingUserSelect.value);
  ...
  if (usersCache.length > 0) {
    const preferred = usersCache.find((u) => u.id === prevSelectedId) || usersCache[0];
    existingUserSelect.value = String(preferred.id);
    fillUserMeasurementFields(preferred);
  } else {
    clearUserMeasurementFields();
  }
}
...
async function loadClothing(userId = null) {
  ...
  if (!Number.isFinite(normalizedUserId) || normalizedUserId <= 0) {
    ...
    fillSelect(
      ...
      [],
      "Hãy chọn user trước khi tải danh sách quần áo.",
    );
    imagePathPreviewInput.value = "";
    return;
  }

  clothingCache = await apiJson(
    `/clothing-items?limit=500&user_id=${normalizedUserId}`,
    {
    method: "GET",
    },
  );
...
async function deleteSelectedUser() {
  ...
  if (!Number.isFinite(userId) || userId <= 0) {
    throw new Error("Please select an existing user to delete");
  }

  const preview = await apiJson(`/users/${userId}/delete-preview`, {
    method: "GET",
  });
  const confirmed = window.confirm(
    `${preview.warning}\n\nBam OK de xac nhan xoa user va toan bo du lieu lien quan.`,
  );
  if (!confirmed) {
    log("Delete user canceled.");
    return;
  }

  const result = await apiJson(`/users/${userId}?confirm=true`, {
    method: "DELETE",
  });
  log(
    `Deleted user #${result.deleted_user_id}. Removed ${result.deleted_clothing_item_count} clothing items and ${result.deleted_body_measurement_count} measurements.`,
  );

  await loadUsers();
  const nextUserId = Number(existingUserSelect.value);
  await loadClothing(nextUserId);
}
...
async function deleteSelectedClothing() {
  ...
  const userId = Number(existingUserSelect.value);
  if (!Number.isFinite(userId) || userId <= 0) {
    throw new Error("Please select an existing user before deleting clothing");
  }
  if (!Number.isFinite(clothingId) || clothingId <= 0) {
    throw new Error("Please select an existing clothing item to delete");
  }

  const item = clothingCache.find((entry) => entry.id === clothingId);
  if (!item || item.user_id !== userId) {
    throw new Error("Selected clothing item does not belong to selected user");
  }

  const confirmed = window.confirm(
    `Ban co chac muon xoa clothing item #${item.id} (${item.display_name || item.category}) cua user #${userId}?`,
  );
  if (!confirmed) {
    log("Delete clothing canceled.");
    return;
  }

  await apiJson(`/clothing-items/${clothingId}?user_id=${userId}`, {
    method: "DELETE",
  });
  log(`Deleted clothing item #${clothingId} of user #${userId}.`);
